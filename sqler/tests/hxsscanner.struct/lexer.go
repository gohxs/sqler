...
func (s *Scanner) token(t TokenType) {
	s.curTok.Typ = t
	s.curTok.Text = func() string { return string(s.src[s.curTok.Pos.Offset:s.srcPos.Offset]) } // States?
}
...
func (s *Scanner) Scan() bool {
	...
	if s.srcPos.Column > 0 {
		...
		s.curTok.Pos.Column = s.srcPos.Column
	} else {
	...
	case eof:
		...
		return false
	case '"':
		s.token(s.scanString('"', TokenDoubleQuote))
	case '\'':
		s.token(s.scanString('\'', TokenSingleQuote))
	case '(':
		s.token(TokenLParen)
	case ')':
		s.token(TokenRParen)
